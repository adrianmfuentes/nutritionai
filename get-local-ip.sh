...
# Script para obtener la IP local y verificar el backend
...
if [[ "$OS" == "Linux" ]]; then
    ...
    # Windows (Git Bash)
    IP=$(ipconfig | grep -A 10 "Wireless LAN adapter Wi-Fi" | grep "IPv4" | awk '{print $NF}')
fi

if [ -z "$IP" ]; then
    echo "❌ No se pudo detectar la IP automáticamente"
    echo ""
    echo "Detecta tu IP manualmente:"
...
# Mostrar contenedores de Docker
echo "🐳 Contenedores Docker:"
docker ps --filter name=nutrition --format "table {{.Names}}\t{{.Status}}\t{{.Ports}}" 2>/dev/null || echo "Docker no está corriendo o no hay contenedores"
echo ""
